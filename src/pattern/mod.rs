...
#[derive(Clone, Debug)]
pub struct CharArraySearcher<'a, const N: usize>(<MultiCharEqPattern<[char; N]> as Pattern<'a>>::Searcher);

/// Associated type for `<&[char; N] as Pattern<'a>>::Searcher`.
#[derive(Clone, Debug)]
pub struct CharArrayRefSearcher<'a, 'b, const N: usize>(<MultiCharEqPattern<&'b [char; N]> as Pattern<'a>>::Searcher);
    ...
    fn new(haystack: &'a str, needle: &'b str) -> StrSearcher<'a, 'b> {
        if needle.is_empty() {
            ...
            StrSearcher {
                haystack,
                needle,
                searcher: StrSearcherImpl::TwoWay(TwoWaySearcher::new(needle.as_bytes(), haystack.len())),
            }
        }
    }
}
    ...
    fn next(&mut self) -> SearchStep {
        match self.searcher {
            ...
            StrSearcherImpl::TwoWay(ref mut searcher) => {
                ...
                match searcher.next::<RejectAndMatch>(self.haystack.as_bytes(), self.needle.as_bytes(), is_long) {
    ...
    fn next_match(&mut self) -> Option<(usize, usize)> {
        match self.searcher {
            ...
            StrSearcherImpl::TwoWay(ref mut searcher) => {
                ...
                if is_long {
                    searcher.next::<MatchOnly>(self.haystack.as_bytes(), self.needle.as_bytes(), true)
                } else {
                    searcher.next::<MatchOnly>(self.haystack.as_bytes(), self.needle.as_bytes(), false)
                }
            }
        }
    }
    ...
    fn next_back(&mut self) -> SearchStep {
        match self.searcher {
            ...
            StrSearcherImpl::TwoWay(ref mut searcher) => {
                ...
                let is_long = searcher.memory == usize::MAX;
                match searcher.next_back::<RejectAndMatch>(self.haystack.as_bytes(), self.needle.as_bytes(), is_long) {
    ...
    fn next_match_back(&mut self) -> Option<(usize, usize)> {
        match self.searcher {
            ...
            StrSearcherImpl::TwoWay(ref mut searcher) => {
                ...
                if is_long {
                    searcher.next_back::<MatchOnly>(self.haystack.as_bytes(), self.needle.as_bytes(), true)
                } else {
                    searcher.next_back::<MatchOnly>(self.haystack.as_bytes(), self.needle.as_bytes(), false)
                }
            }
        }
    }
...
impl TwoWaySearcher {
        ...
        S: TwoWayStrategy,
    {
        ...
        'search: loop {
            ...
            let start = if long_period { self.crit_pos } else { cmp::max(self.crit_pos, self.memory) };
